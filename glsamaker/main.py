...
def get_xml_text_lines(xml_root, match):
    ...
    return "\n".join(list(filter(None, [line.strip() for line in text.splitlines()])))
...
def xml_to_glsa(xml):
    ...
    for package in root.find("affected").findall("package"):
        ...
        for tag in package:
            ...
            glsa.affected.append(
                Affected(pkg, version, atom_range, arch, slot, range_type)
            )
    ...
    for uri in root.find("references"):
        glsa.references.append(
            Reference(
                uri.text.strip(), uri.attrib["link"] if "link" in uri.attrib else None
            )
        )
    ...
    if requested_tag and "timestamp" in requested_tag:
        glsa.requested_time = datetime.fromisoformat(
            requested_tag["timestamp"].rstrip("Z")
        )

    submitted_tag = get_xml_attrib(root, './/metadata[@tag="submitter"]')
    if submitted_tag and "timestamp" in submitted_tag:
        glsa.submitted_time = datetime.fromisoformat(
            submitted_tag["timestamp"].rstrip("Z")
        )
...
def populate_glsa_db():
    glsa_xmls = [f for f in os.listdir("glsa") if f.endswith(".xml")]
